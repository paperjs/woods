...
var hashFile = function (file, callback) {
	...
	var readStream = fs.createReadStream(file.file);
...
var exportSite = function(site, save, done) {
	var pageQueue = async.queue(function(page, done) {
		...
		save(page, url.resolve(page.url, 'index.html'), done);
	}, 10);
...
Site.prototype.syncS3 = function(callback) {
	// Try to read keys from environment if not specified in the settings
	this.settings.s3key = this.settings.s3key || process.env.AWS_ACCESS_KEY_ID;
	this.settings.s3secret = this.settings.s3secret || process.env.AWS_SECRET_ACCESS_KEY;

	if (!this.settings.s3key || !this.settings.s3secret || !this.settings.s3bucket)
		return callback(Error('S3 configuration not found. Missing one or more of s3key, s3secret and s3bucket settings in site-directory/settings.json'));
	...
	var putHtml = function(page, callback) {
		...
		if (s3List[uri] && s3List[uri].hash == '"' + hashBuffer(buffer) + '"') {
			s3List[uri].uploaded = true;
			callback();
		} else {
			client.putBuffer(buffer, uri, s3Options, callback);
		}
	...
	listFiles(function(err) {
		...
		exportSite(site, put, function(err) {
			if (err) return callback(err);
			var toDelete = [];
			for (let uri in s3List) {
				if (!s3List[uri].uploaded) {
					toDelete.push(uri);
				}
			}
