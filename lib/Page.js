...
var Page = function(site, parent, directory, index, callback) {
	...
	var page = this;
	addFiles(this, function() {
		addChildren(page, function(err) {
			if (page._contentFile) {
				page.setType(path.basename(page._contentFile, '.md'), function(err) {
					if (err)
						return callback(err);
...
Page.prototype.setType = function(type, callback) {
	this.type = type;
	this.site.addType(type, callback);
};
...
Page.prototype.render = function(name, param) {
	param = getTemplateParam(this, param);
...
var getTemplateParam = function(page, param) {
	var templateParam = {
		page: page,
		site: page.site,
		root: page.site.root,
		...
		render: function(template, page) {
			return (page || templateParam.page).render(template, templateParam);
		}
	};
	for (var key in param) {
		if (key != 'page' && !templateParam[key] && param.hasOwnProperty(key)) {
			templateParam[key] = param[key];
		}
	}
...
var addChildren = function(page, callback) {
